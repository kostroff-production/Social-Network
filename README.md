...
<h3>API</h3>
Практически 80% запросов к базе данных происходит без перезагрузки страницы, что ускоряет ответ от сервера и улучшает восприятие сайта.<br> 
В этом мне помог фреймоврк ```Django Rest Framework```.<br>
Для примера продемострирую фрагмент кода, который решает проеблему подгрузки последнего созданного сообщения пользователем:
```
...
Установка и документация описана на сайте: ```https://www.django-rest-framework.org/```
